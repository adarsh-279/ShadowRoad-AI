...
const DriveLegal = () => {
  ...
  const [messages, setMessages] = useState([
    { type: 'bot', text: "Namaste! I'm your DriveLegal AI assistant. Ask me anything about Indian road laws, challans, or your rights as a driver." }
  ]);
  ...
  const quickQuestions = [
    { label: 'No license fine?', q: 'What is the fine for driving without license?' },
    { label: 'Fight challan?', q: 'Can I fight a wrong challan?' },
    { label: 'School zone speed?', q: 'What is the speed limit in school zones?' },
    { label: 'Helmet law?', q: 'Do I need a helmet on a bike in India?' }
  ];
    ...
    <section className="section" id="drivelegal">
      <div className="container">
        ...
        <div className="legal-grid">
          <div className="legal-card" style={{opacity: 1, transform: 'none'}}>
            ...
            <button className="legal-try-btn" onClick={() => setChatOpen(true)}>💬 Ask AI Legal Assistant</button>
          </div>
        ...
        {chatOpen && (
          <div className="modal-overlay active" onClick={(e) => { if(e.target.className === 'modal-overlay active') setChatOpen(false) }}>
            <div className="modal-box">
              ...
              <div className="chat-messages">
                {messages.map((msg, idx) => (
                  <div key={idx} className={`chat-msg ${msg.type === 'bot' ? 'bot-msg' : 'user-msg'}`}>
                    <div className="chat-avatar">{msg.type === 'bot' ? '⚖️' : '👤'}</div>
              ...
              <div className="chat-quick-btns">
                {quickQuestions.map((q, i) => (
                  <button key={i} className="quick-btn" onClick={() => handleSend(q.q)}>{q.label}</button>
                ))}
              </div>
              <div className="chat-input-row">
                ...
                />
                <button className="chat-send" onClick={() => handleSend(inputValue)}>➤</button>
              </div>
            </div>
          </div>
        )}
